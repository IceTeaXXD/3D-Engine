import { Node, Mesh } from "../../core/index.js"
...
export class ArticulatedModel extends Node {
  /** @type {{[k: string]: Rig}} */
  #rigs = {}
  #materials = {}
  ...
  static getRigs(obj) {
    ...
    obj.children.forEach((child) => {
      ...
      if (child instanceof Rig) {
        if (child.id in rigs)
          throw new Error(`Rig with id ${child.id} already exists in model.`)
        rigs[child.id] = child
      }
    })
    return rigs
  ...
  static fromJSON(json, obj = null) {
    ...
    Object.keys(json.materials).forEach((matName) => {
      obj.#materials[matName] = json.materials[matName].map((mat) =>
        MaterialDeserializer(mat)
      )
    })
  ...
  static fromModelDefinition(modelDefinition, model = null, parent = null) {
    ...
    Object.keys(modelDefinition).forEach((objName) => {
      ...
      if (objName[0] == "R") {
        ...
        model.#materials[objName] = [new PhongMaterial(), new BasicMaterial()]
        child = new Mesh(
      ...
      if (md.children) this.fromModelDefinition(md.children, model, child)
    })
    return model
  }
}
